...
const getOrderDetails = async (req, res) => {
  ...
  try {
    const order = await Order.findOne({ _id: orderId, userId: req.session.user.userId });
    if (!order) {
    ...
    const validOrderDetails = orderDetails.filter(detail => detail.productId !== null);

    const total = validOrderDetails.reduce((sum, detail) => sum + detail.quantity * detail.productId.price, 0);

    res.render("Order/details", { order, orderDetails: validOrderDetails, total });
  } catch (error) {
